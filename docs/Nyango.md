...
2. 敵に接触するとライフ減少
3. 敵を全滅するとステージクリア（毛玉を3つそろえるとフィールドに「チュール」が出現）
4. ステージごとに制限時間あり
...
* **勝利条件**
  ...
  * 敵を全滅させる
  * （補足）毛玉を3つ揃えると「チュール」が出現し、取得でボーナス。クリア条件ではない。
* **敗北条件**
...
* **通路骨格**：内部は砂山でほぼ埋め、通路は一筆書きを2〜3回行って複数の分岐なし通路を生成（自己交差なし）。
* **砂山配置**：外周を除く内部は基本砂山。上記の単一路のみを掘削して通路化する。
* **毛玉配置**：常に5〜10個配置。初期配置では毛玉同士が隣接（上下左右）しないように通路上に置く（3揃えはプレイヤーの押し操作で作る）。
* **プレイヤー配置**：開始位置は外周木から近すぎず、通路の端点付近に設定。
* **敵配置**：
